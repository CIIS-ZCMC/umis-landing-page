...
const HealthNewsItems = [
    {
        date: 'August 10, 2023',
        title: 'MPOX Update',
        description: 'Important Update on Monkeypox (Mpox) in the Philippines. The Department of Health (DOH)...',
        image: env
    },

    {
        ...
        description: 'The Department of Health has released new guidelines for COVID-19 safety...',
        image: env
    },

    {
        ...
        description: 'The latest statistics on the vaccination drive across the country...',
        image: env
    },

    {
        ...
        description: 'New initiatives to improve the healthcare system have been announced...',
        image: env
    },
...
const HealthNewsItem = () => {
    ...
    const handleNext = () => {
        setCurrentIndex(prevIndex => Math.min(prevIndex + itemsPerPage, HealthNewsItems.length - itemsPerPage));
    };

    const displayedItems = HealthNewsItems.slice(currentIndex, currentIndex + itemsPerPage);
    ...
    return (
        <div className="content-2">
            <div className="list-heading">
                <div className="text-wrapper-9">Latest releases</div>

                <div className="frame-2">
                    <button className={`img-wrapper ${isPreviousDisabled ? 'disabled' : ''}`} onClick={handlePrevious} disabled={isPreviousDisabled}>
                        <svg className={`img img-previous ${isPreviousDisabled ? 'disabled' : ''}`} width="24" height="24" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <button className={`img-wrapper ${isNextDisabled ? 'disabled' : ''}`} onClick={handleNext} disabled={isNextDisabled}>
                        <svg className={`img img-next ${isNextDisabled ? 'disabled' : ''}`} width="24" height="24" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className="div-3">
                    ...
                    {displayedItems.map((item, index) => (
                        <div className="list-item-2" key={index}>
                            <div className="image-wrapper">
                                <img className="recent-news-img" src={item.image} alt={item.title} />
                            </div>
                            <div className="container-4">
                                <div className="div-4">
                                    <div className="text-wrapper-7">Published {item.date}</div>
                ...
                <a className="secondary-CTA" href="#" rel="noopener noreferrer">
                    All recent news (+{HealthNewsItems.length - (currentIndex + itemsPerPage)} more)
                </a>
            </div>
        </div>
    );
